...
to examplify modelling. Please see the examples in `owl()`.) The main
...
in @bogdan2015 to select the sequence. (Note that it is also posssible
...
true signals. Under this gaussian design with indetically
and independently distributed columns in $X$, SLOPE should asymptotically
control FDR at the level given by the shape parameter $q$, which
we set to 0.2 in this example.
...
for (i in seq_along(q)) {
  ...
  selected_owl <- which(abs(coef(fit)) > 0)
...
SLOPE seems to accomplish its target very well, although it is too conservative
when the number of true signals is high.
