...
test_that("erroneous group slope input throws errors", {
  ...
  expect_error(golem(x, y, groups = groups, penalty = "group_slope", sigma = -1))
  expect_error(golem(x, y, groups = groups, penalty = "group_slope",
                     lambda = 1:2, sigma = 1))
  expect_error(golem(x, y, groups = groups, penalty = "group_slope",
                     lambda = c(3, 1, -1)))
...
test_that("erroneous slope input throws errors", {
  ...
  expect_error(golem(x, y, penalty = "slope", lambda = -c(1, 2)))
  expect_error(golem(x, y, penalty = "slope", lambda = 4:1))
})
